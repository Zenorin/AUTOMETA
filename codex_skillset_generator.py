...
def compute_skill_plan(profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workstream_required = workstream_skill_names(profile) if profile.get("generate_workstream_skills", True) else []
    required = unique(core + policy_required + explicit_required + workstream_required)
...
def normalize_workstream(raw: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize a project-specific workstream into a first-class custom skill spec."""
    if not isinstance(raw, dict):
        raise ValueError("workstream entries must be objects")
    workstream = dict(raw)
    name = str(workstream.get("name") or workstream.get("title") or "").strip()
    skill = str(workstream.get("skill") or workstream.get("slug") or slugify_skill_name(name)).strip()
    if not skill:
        raise ValueError("workstream.skill or workstream.name is required")
    workstream["skill"] = skill
    workstream.setdefault("name", slug_title(skill))
    workstream.setdefault("objective", workstream.get("description", f"Own the {workstream['name']} workstream."))
    module_paths = workstream.get("module_paths", workstream.get("paths", []))
    if isinstance(module_paths, str):
        module_paths = [module_paths]
    workstream["module_paths"] = list(module_paths or [])
    workstream.setdefault("use_when", [])
    workstream.setdefault("workflow", [])
    workstream.setdefault("quality_gates", [])
    workstream.setdefault("references", [])
    workstream.setdefault("forbidden", [])
    workstream.setdefault("commands", {})
    workstream.setdefault("required_skills", [])
    return workstream


def workstream_skill_names(profile: Dict[str, Any]) -> List[str]:
    return unique([w.get("skill", "") for w in profile.get("workstreams", [])])


def workstream_by_skill(profile: Optional[Dict[str, Any]], skill: str) -> Optional[Dict[str, Any]]:
    if not profile:
        return None
    for workstream in profile.get("workstreams", []):
        if workstream.get("skill") == skill:
            return workstream
    return None


def workstream_matches_module(workstream: Dict[str, Any], module: Dict[str, Any]) -> bool:
    module_path = str(module.get("path", "")).replace("\\", "/").strip("/")
    if not module_path:
        return False
    for raw_path in workstream.get("module_paths", []):
        path = str(raw_path).replace("\\", "/").strip("/")
        if not path:
            continue
        if path == module_path or path.startswith(module_path + "/") or module_path.startswith(path + "/"):
            return True
    return False


def attach_workstreams_to_modules(profile: Dict[str, Any]) -> None:
    for module in profile.get("modules", []):
        routed = [w["skill"] for w in profile.get("workstreams", []) if workstream_matches_module(w, module)]
        if routed:
            module["preferred_skills"] = unique(list(module.get("preferred_skills", [])) + routed)


def render_workstream_skill(workstream: Dict[str, Any]) -> str:
    skill = workstream["skill"]
    desc = str(workstream.get("objective") or f"Own the {workstream.get('name', skill)} workstream.")
    lines = [
        "---",
        f"name: {yaml_quote(skill)}",
        f"description: {yaml_quote(desc[:240])}",
        "---",
        "",
        f"# {workstream.get('name') or slug_title(skill)}",
        "",
        "## Objective",
        desc,
        "",
    ]
    if workstream.get("module_paths"):
        lines += ["## Routed paths"] + [f"- `{path}`" for path in workstream["module_paths"]] + [""]
    if workstream.get("use_when"):
        lines += ["## Use when"] + [f"- {item}" for item in workstream["use_when"]] + [""]
    if workstream.get("required_skills"):
        lines += ["## Coordinate with"] + [f"- `${item}`" for item in workstream["required_skills"]] + [""]
    if workstream.get("workflow"):
        lines += ["## Workflow"] + [f"{idx}. {item}" for idx, item in enumerate(workstream["workflow"], start=1)] + [""]
    else:
        lines += [
            "## Workflow",
            "1. Identify the exact feature, contract, files, and non-goals.",
            "2. Read the closest `AGENTS.md`, relevant references, and existing tests before editing.",
            "3. Make the smallest coherent change that preserves existing behavior.",
            "4. Add or update tests, fixtures, docs, or evidence for changed behavior.",
            "5. Report changed files, commands run, PASS/FAIL evidence, and remaining risks.",
            "",
        ]
    if workstream.get("commands"):
        lines += ["## Commands"] + [f"- `{key}`: `{value}`" for key, value in workstream["commands"].items()] + [""]
    if workstream.get("references"):
        lines += ["## References"] + [f"- `{item}`" for item in workstream["references"]] + [""]
    if workstream.get("quality_gates"):
        lines += ["## Quality gates"] + [f"- {item}" for item in workstream["quality_gates"]] + [""]
    else:
        lines += ["## Quality gates", "- Relevant validation commands pass or blockers are documented.", ""]
    if workstream.get("forbidden"):
        lines += ["## Forbidden"] + [f"- {item}" for item in workstream["forbidden"]] + [""]
    lines += [
        "## Evidence required",
        "- Files read and changed.",
        "- Commands run and outcomes.",
        "- Tests, fixtures, screenshots, or contract evidence where relevant.",
        "- Known risks and explicit exclusions.",
        "",
        "## Handoff",
...
def render_workstream_split_guide(profile: Dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "Workstreams are first-class project-specific skills. Use them when generic bundles are too broad for the actual development lanes.",
        "",
        "## Why this exists",
        "The generator keeps the universal skill list compact for routing reliability. For projects with distinct lanes, define `workstreams` so each lane becomes an explicit SKILL.md instead of staying as vague module prose.",
        "",
    ]
    if profile.get("workstreams"):
        lines.append("## Configured workstreams")
        for w in profile["workstreams"]:
            lines.append(f"### `${w['skill']}` — {w.get('name')}")
            lines.append(f"- objective: {w.get('objective')}")
            lines.append(f"- paths: {', '.join('`'+path+'`' for path in w.get('module_paths', [])) or 'none'}")
            lines.append("")
    else:
        lines += ["## Configured workstreams", "- None.", ""]
    lines += [
        "## When to add a workstream",
        "- A module has multiple distinct job types, such as UI design, state handling, API contracts, fixtures, and release checks.",
        "- A reference-heavy task requires a repeatable clean-room analysis lane.",
        "- A project has tool-selection branches where Codex must not choose the easiest path silently.",
        "- A user expects a task lane to be directly invokable with `$skill-name`.",
        "",
        ...
        "      \"objective\": \"Own UI composition, state coverage, responsive layout, and smoke checks.\",",
        "      \"workflow\": [\"Review component boundaries.\", \"Implement without dropping states.\", \"Run UI checks.\"],",
        "      \"quality_gates\": [\"Build passes.\"]",
        "    }",
        "  ]",
        "}",
        "```",
...
def normalize_profile(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    profile.setdefault("generate_workstream_skills", True)
    ...
    attach_workstreams_to_modules(profile)
    return profile
...
def validate_profile(profile: Dict[str, Any]) -> List[str]:
    ...
    workstream_skills = set(workstream_skill_names(p))
    known = all_known_skills() | workstream_skills
    seen_workstream_skills: Set[str] = set()
    for workstream in p.get("workstreams", []):
        skill = workstream.get("skill", "")
        if not is_safe_skill_name(skill):
            errors.append(f"Unsafe workstream skill name: {skill}. Use lowercase slug form such as `project-frontend-design`.")
        if skill in seen_workstream_skills:
            errors.append(f"Duplicate workstream skill name: {skill}")
        seen_workstream_skills.add(skill)
        for path in workstream.get("module_paths", []):
            if not is_safe_relative_path(path):
                errors.append(f"Unsafe workstream module_path in {skill}: {path}. Use repository-relative paths only.")
        if not isinstance(workstream.get("commands", {}), dict):
            errors.append(f"workstream.commands must be an object in {skill}")
        for linked_skill in workstream.get("required_skills", []):
            if not is_safe_skill_name(linked_skill):
                errors.append(f"Unsafe workstream required_skill in {skill}: {linked_skill}")
...
def skill_description(skill: str, profile: Optional[Dict[str, Any]] = None) -> str:
    workstream = workstream_by_skill(profile, skill)
    if workstream:
        return str(workstream.get("objective") or f"Own the {workstream.get('name', skill)} workstream.")
    return SKILL_DESCRIPTIONS.get(skill, f"Use this skill for project-specific workflow `{skill}`.")


def render_skill(skill: str, profile: Optional[Dict[str, Any]] = None) -> str:
    workstream = workstream_by_skill(profile, skill)
    if workstream:
        return render_workstream_skill(workstream)
    desc = skill_description(skill, profile)
...
def render_openai_yaml(skill: str, profile: Optional[Dict[str, Any]] = None) -> str:
    workstream = workstream_by_skill(profile, skill)
    desc = skill_description(skill, profile)[:160]
    allow = "false" if skill in EXPLICIT_ONLY_SKILLS else "true"
    display = workstream.get("name") if workstream else slug_title(skill)
    return f"""interface:
  display_name: {yaml_quote(display)}
...
def render_agents_md(profile: Dict[str, Any], skills: Sequence[str], opts: Dict[str, Any]) -> str:
    ...
    lines.append("")
    if profile.get("workstreams"):
        lines.append("## First-class workstreams")
        lines.append("Use these project-specific skills when a task maps to a concrete development lane rather than a generic bundle.")
        for w in profile["workstreams"]:
            lines.append(f"- `${w['skill']}` → {w.get('objective')}")
        lines.append("")
...
def render_policy_lock(profile: Dict[str, Any]) -> str:
    lock = {"version": VERSION, "project_name": profile["project_name"], "project_type": profile["project_type"], "project_size": profile["project_size"], "bundles": profile.get("bundles", []), "policy_packs": profile.get("policy_packs", []), "modules": [{"name": m["name"], "path": m["path"], "module_type": m["module_type"], "bundles": m.get("bundles", [])} for m in profile.get("modules", [])], "workstreams": [{"skill": w["skill"], "module_paths": w.get("module_paths", [])} for w in profile.get("workstreams", [])], "allowed_external_hosts": profile.get("allowed_external_hosts", []), "note": "Hard policies are opt-in via policy_packs, not universal defaults."}
        ...
        "workstreams": [
            {
                "skill": w["skill"],
                "name": w.get("name"),
                "module_paths": w.get("module_paths", []),
...
5. Define workstreams when project lanes need first-class custom skills beyond generic bundles.
6. Generate the skillset and run `validate-generated`.
"""
  ...
  "workstreams": [
    {
      ...
      "name": "Project Frontend Design",
      "module_paths": ["apps/web"],
      "objective": "Own a concrete project work lane as a first-class SKILL.md.",
      "workflow": ["Read relevant docs and module AGENTS.md.", "Implement the smallest complete slice.", "Run validation and report evidence."],
      "quality_gates": ["Relevant checks pass."],
      "forbidden": ["Do not delete behavior without role trace or approval."]
    }
  ]
}
```
...
def generate(profile: Dict[str, Any], root: Path, mode: str = "skip", also_codex_skills: bool = False) -> None:
    ...
    for skill in skills:
        write_file(root/".agents"/"skills"/skill/"SKILL.md", render_skill(skill, profile), mode)
        write_file(root/".agents"/"skills"/skill/"agents"/"openai.yaml", render_openai_yaml(skill, profile), mode)
        if also_codex_skills:
            write_file(root/".codex"/"skills"/skill/"SKILL.md", render_skill(skill, profile), mode)
            write_file(root/".codex"/"skills"/skill/"agents"/"openai.yaml", render_openai_yaml(skill, profile), mode)
    ...
    write_file(root/"docs"/"skillset"/"workstream-split-guide.md", render_workstream_split_guide(profile), mode)
